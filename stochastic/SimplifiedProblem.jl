...
#this function is reading the


# function hyperplanes from a textfile
...
function create_optimization_model()
      ...
      for h in power_plants[i].hyperplanes[power_plants[i].maxTurbines]
        ...
        if i==2
          if t in periods_optimizables_ccs
            ...
            for w in 1:scenario_number
              if(h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][1]||h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][25]||h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][15]||
                h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][10]||
                h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][20]||
                h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][8]||
                h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][5])
                @constraint(model, production[i,t,w]<=h.z+h.x*discharge_water[i,t,w]+h.y*reservoir_volume[i,t,w])
              end
            end
          end
        ...
        if i==3
          if t in periods_optimizables_cim
            ...
            for w in 1:scenario_number
              if(h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][1]||h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][25]||h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][15]||h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][5]||h==power_plants[i].hyperplanes[power_plants[i].maxTurbines][10])
                @constraint(model, production[i,t,w]<=h.z+h.x*discharge_water[i,t,w]+h.y*reservoir_volume[i,t,w])
              end
            end
  ...
  for m in 1:size(maintenances)[1]
    ...
    for i in 1:size(power_plants)[1]
      ...
      savefig(string(config["globalParameters"]["savingResultsFolder"],ARGS[1],"/MaintenancesGeoffrey.png"))
      PowersFile =  open(string(config["globalParameters"]["savingResultsFolder"],"/Powers.txt"),"a")

      write(PowersFile, string(totalPower,","));
      TimeFile =  open(string(config["globalParameters"]["savingResultsFolder"],"/Time.txt"),"a")
    ...
    CSV.write(string(config["globalParameters"]["savingResultsFolder"],ARGS[1],"/results.csv"), df)
    ...
    savefig(string(config["globalParameters"]["savingResultsFolder"],ARGS[1],"/ReservoirVolumeALL.png"))
    #Dischargre water
    ...
    savefig(string(config["globalParameters"]["savingResultsFolder"],ARGS[1],"/DischargedWaterALL.png"))
    ...
    savefig(string(config["globalParameters"]["savingResultsFolder"],ARGS[1],"/EstimatedPowerALL.png"))
    #true Production
